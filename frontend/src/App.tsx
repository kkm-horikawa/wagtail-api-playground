...
function GraphQLSection() {
  ...
  return (
    <div className="section">
      ...
      <ul>
        {data.pages.map((page: GraphQLPage) => (
          <li key={page.id}>
            <strong>{page.title}</strong>
  ...
  return (
    <div className="app">
      ...
      <div className="info">
        <h3>Backend URLs</h3>
        <ul>
          <li><a href={`${API_BASE}/admin/`} target="_blank">Wagtail Admin</a></li>
          <li><a href={`${API_BASE}/api/v2/pages/`} target="_blank">REST API</a></li>
          <li><a href={`${API_BASE}/graphql/`} target="_blank">GraphQL Playground</a></li>
        </ul>
      </div>
    </div>
  )
